...
class Lending_Pool(ERC_20.ERC_20, Protocol_Data_Provider.Protocol_Data_Provider):
    ...
    def run_all_lend_event_tracking(self):
        ...
        receipt_token_list = ['0xe3f709397e87032E61f4248f53Ee5c9a9aBb6440', '0x083E519E76fe7e68C15A6163279eAAf87E2addAE']
        ...
        block_df = pd.read_csv('./config/leverager_block_number.csv')
        block_df['block_number'] = block_df['block_number'].astype(float)

        max_leverage_block = block_df['block_number'].max()

        leverager_block_number_list = block_df['block_number'].unique()

        blocks_scanned = 0
        # while to_block < latest_block:

        # # tries to set small ranges around our blocks
        for block_number in leverager_block_number_list:
            from_block = block_number
            to_block = block_number + self.interval   
            ...
            while i < len(receipt_token_list):
                ...
                if len(events) > 0:
                    ...
                    if len(contract_df) > 0:
                        ...
                        print(contract_df['tx_hash'].tolist())
            ...
            print('Current Event Block vs Latest Event Block to Check: ', from_block, '/', max_leverage_block, 'Blocks Remaining: ', len(leverager_block_number_list) - blocks_scanned)
            time.sleep(wait_time)
